...
/* per-bundle setup: sort + clean so the state-filtered analysis has the cleaned dataset */
Proc sort data=claims_cleaned out=new_claims_cleaned; by Incident_Date; run;
...
Data new_claims_cleaned;
    ...
    If (Incident_Date=. or Date_Received=.
        or Year(Incident_Date)<2002 or Year(Date_Received)<2002
        or Year(Incident_Date)>2017 or Year(Date_Received)>2017)
